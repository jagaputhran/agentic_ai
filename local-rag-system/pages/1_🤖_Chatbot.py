...
st.set_page_config(page_title="JagAI - Chatbot", page_icon="🤖")
...
def render_chatbot_page() -> None:
    ...
    st.title("JAGA(Just-in-time AI for Guided Assistance)⭐🤖")
    
    # Display current model being used
    st.info(f"🤖 **LLM Model:** {OLLAMA_MODEL_NAME} | 🔍 **Embedding:** sentence-transformers/all-mpnet-base-v2")
    ...
    st.sidebar.markdown(
        "<h2 style='text-align: center;'>AI with Jaga</h2>", unsafe_allow_html=True
    )
    st.sidebar.markdown(
    ...
    if "embedding_models_loaded" not in st.session_state:
        with model_loading_placeholder:
            with st.container():
                ...
                status_text.markdown("⏳ **Loading embedding model (this may take 30-60 seconds on first load)...**")
                progress_bar.progress(25)
                get_embedding_model()
                
                status_text.markdown("⏳ **Checking Ollama model availability...**")
